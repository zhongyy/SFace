...
def get_dataset_test(input_dir):
  print("test")
  ret = []
  label = 0
  img_names = []
...
#this code can be used to use json file in challenge2
'''
def get_dataset_challenge2(input_dir,json_dir):
  ret = []
  label = 0
  for prefixdir in os.listdir(input_dir):
    _prefixdir = os.path.join(input_dir, prefixdir)
    for img in os.listdir(_prefixdir):
        if not img.endswith('.jpg.jpg') and img.endswith('.jpg'):
          fimage = edict()
          fimage.id = os.path.join(prefixdir, img)
          fimage.classname = str(label)
          fimage.image_path =os.path.join(_prefixdir,img)
          json_path,b=fimage.id.split(".")
          ...
          if os.path.exists(json_file):
            ...
            if 'landmarks' in data:
              ...
              fimage.landmark = np.zeros( (3,2), dtype=np.float32 )
              fimage.landmark[0][0] = float(landmarks[94].split("(")[1])#left of mouth48
              fimage.landmark[0][1] = float(landmarks[95].split(")")[0])#left of mouth
              fimage.landmark[1][0] = float(landmarks[106].split("(")[1])#right of mouth54
              fimage.landmark[1][1] = float(landmarks[107].split(")")[0])#right of mouth
              fimage.landmark[2][0] = float(landmarks[58].split("(")[1])#nose 30
              fimage.landmark[2][1] = float(landmarks[59].split(")")[0])#nose
              #print('lm')
  ...
  return ret
'''
def get_dataset_challenge2(input_dir):
  ret = []
  label = 0
  for prefixdir in os.listdir(input_dir):
    _prefixdir = os.path.join(input_dir, prefixdir)
...
def get_dataset(name, input_dir,json_dir=0):
  ...
  if name == 'challenge2':
    return get_dataset_challenge2(input_dir)
  if name=='megaface':
